...
#csv files to db
csv_projects = open('D://Python_Learn//csv_projects.csv', 'r') #file with projects
csv_tasks = open('D://Python_Learn//csv_tasks.csv', 'r') #file with tasks
...
cursor = db.tasks.find({"status": "canceled"})
for record in cursor:
    print(record)

print("**************** Projects with status done *********************************")
#tasks join projects on name = project
cursor_tasks = db.projects.aggregate([
 ...
 #{  "$unwind": "$items" }
 ,{"$match":{"task_name.status": "canceled"} } , #where condition
 {"$project":{ #$project - for filtering to show only the fields we need:<field> : <1 or true>
  "name":1
 }
 }
])
